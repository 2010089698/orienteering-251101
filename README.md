...
1. **データベース初期化**
   - コマンドによる即時初期化: `npm run db:reset` を実行すると、`backend/.env.development` の設定を読み込みつつ SQLite データベースを単発で再作成します。バックエンドサーバーを起動していない状態でも安全にデータストアをリセットできます。
   - 自動初期化フラグ: `backend/.env.development` の `RESET_DB=true` を設定してバックエンドを再起動すると、起動時に自動でデータベースが再作成されます。継続運用時は `RESET_DB=false` に戻し、意図しない再初期化を防ぎます。
